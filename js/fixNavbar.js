...
if ($(document).width() > 360) { // Required if "viewport" content is "width=device-width, initial-scale=1.0": navbar is not fixed on lower widths.
    ...
    if (hash != "") {
        $(document).scrollTop(($(hash).offset().top) - ($(".navbar").height() * 1.1));
    }
    ...
    function setNewOnClick(originalOnClick, hash) {
        anchorsList[i].onclick = function () {
            $(originalOnClick);
            $(document).scrollTop(($(hash).offset().top) - ($(".navbar").height() * 1.7));
            return false;
        };
    }
}
